...
if(!isset($_POST["submit"])){
    ...
    if($total_price==0){
        $stmt = $db->query("INSERT INTO `payments` (id, email, book_ids, amount, date, transaction_id) VALUES (0, ?, ?, ?, ?, '')", "ssds", [$email, $ids, $total_price, $date]);
        if($stmt){
    ...
    $stmt = $db->query("INSERT INTO `payments` (id, email, book_ids, amount, date, transaction_id) VALUES (0, ?, ?, ?, ?, ?)", "ssdss", [$email, $ids, $amount, $date, $trans_id]);
    if($stmt){
        $stmt2 = $db->query("UPDATE `cart` SET `status`=1 WHERE `user_email`=? AND FIND_IN_SET(book_id, ?)", "ss", [$email, $ids]);
        if($stmt2){
            ?>
